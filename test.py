...
if __name__ == '__main__':
    ...
    for t in sub_list:
        ...
        if view == 'ax':
            ...
            raise NotImplementedError('View {} is not recognized.'.format(view))
        ...
        for i in range(1, total_slice - 1):
            ...
            if np.sum(lesion_slice) > 0:
                if view == 'ax':
                    ...
                    for j in range(3):
                        b_temp, m_temp, dim1_pad, dim2_pad = pad(b_slice[:, :, j], mask_slice[:, :, j], x, y, opt.pad_to_size)
                    ...
                    mask_slice = lesion_mask[:, i - 1:i + 2, :]
                    for j in range(3):
                        b_temp, m_temp, dim1_pad, dim2_pad = pad(b_slice[:, j, :], mask_slice[:, j, :], x, z, opt.pad_to_size)
                        b_slices.append(b_temp)
                        mask_slices.append(m_temp)
                else:
                    raise NotImplementedError('View {} is not recognized.'.format(view))
                ...
                if view == 'ax':
                    result_slice = unpad(temp, x, dim1_pad, y, dim2_pad)
                    b_slice = brain_img[:, :, i]
                    ...
                    result_slice = unpad(temp, x, dim1_pad, z, dim2_pad)
                    b_slice = brain_img[:, i, :]
                    mask_slice = lesion_mask[:, i, :]
                    network_img[:, i, :] = result_slice
                    result_slice = (1-mask_slice) * b_slice + mask_slice * result_slice
        ...
        result_img_.to_filename(os.path.join(opt.dataroot, opt.name, str(s) + '_brain_inpainted.nii.gz'))
        if opt.raw_output:
            net_img_ = nib.Nifti1Image(network_img, affine=aff1)
            net_img_.to_filename(os.path.join(opt.dataroot, opt.name, str(s) + '_network_raw.nii.gz'))
